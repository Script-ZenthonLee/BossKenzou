...
local function fetchBlacklist()
    ...
    if not fetchResult[1] then
        return false, "BLACKLIST_FETCH_FAILED"
    end

    local blacklistData = fetchResult[2]
    local loadResult = {
        pcall(function()
            return loadstring(blacklistData)()
        end)
    }

    if not loadResult[1] or type(loadResult[2]) ~= "table" then
        return false, "INVALID_BLACKLIST_DATA"
    end

    blacklist = loadResult[2]
    return true
end

local fetchSuccess, fetchError = fetchBlacklist()
if not fetchSuccess then
    LocalPlayer:Kick(fetchError or "Blacklist fetch failed")
    return
end
...
local function isBlacklisted()
    ...
    -- Check dictionary format (Key-Value)
    if blacklist[playerName] == true or blacklist[userId] == true or blacklist[tostring(userId)] == true then
        return true
    end
    
    -- Check array format (List of values)
    for _, identifier in ipairs(blacklist) do
        if identifier == playerName or identifier == userId or tostring(identifier) == tostring(userId) then
            return true
        end
    end
